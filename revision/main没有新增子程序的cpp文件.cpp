...
int N = 7;
const int Nx = 100; // the x-axis grid numbers
const int Ny = 80; // the y-axis grid numbers
...
int main()
{
    ...
    cout << "First Step: begin to calculate " << N << " grains growth with kim algorithm!" << endl;
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
    ...
    double garma = 0.208, //  J/m2
           allTime = 50.0,   // the whole time to grow    
           deltaE = 0.09,
           deltaX = 0.5e-6,
           Qb = 110e3,       //  j/mol
           R = 8.314,           //  j/(K*mol)
    ...
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
    cout << "Epsilon is " << epsilon << "/s, " << "deltaT is " << deltaT << "." << endl;    
