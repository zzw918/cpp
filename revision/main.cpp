...
// 创建位错密度结构体，key为晶粒标号，rho为对应的位错密度
struct GrainRho
{
    int key;
    double rho;
...
int N = 7;
const int Nx = 447; // the x-axis grid numbers
const int Ny = 258;  // the y-axis grid numbers
...
int main()
{
    cout << "This program includes two steps: Grain growth and DRX." << endl;
    ...
    cout << "Parameters: Nx = " << Nx << ", Ny = " << Ny << endl << endl;
    
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY | FOREGROUND_GREEN);
    cout << "First Step: begin to calculate " << N - 1 << " grains growth with kim algorithm!" << endl;
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
    ...
    double intervalTimeGrowth = 5.0;

    cout << "deltaT is " << deltaT << ", all files will be created every " << intervalTimeGrowth/deltaT << " steps."<< endl;
    cout << "Please wait to compute..." << endl;

    double curTime = 0.0;
    int number = 1; // filename number mark

    double timeFlagGrowth = 0.0;

    // 此参数可以设置，即每隔intervalTimeGrowth时间生成一个文件。
    ...
    while (true)
    {
        ...
        // time added
        curTime += deltaT;
        timeFlagGrowth += deltaT;

        if (timeFlagGrowth > intervalTimeGrowth)
        {
            cout << number << " growth files have been created, current time is " << int(curTime) << "." << endl;
            output(phi_o, grid_b, it_b, N, Nx, Ny, number++);
            timeFlagGrowth = 0;

            // 这里判断基体晶粒是否完全消失，如果完全消失，说明晶粒已经长大，那么就可以跳出循环，做再结晶的工作了。
            int baseNum = 0;
            for (int i = 0; i < Nx; i++) {
                for (int j = 0; j < Ny; j++) {
                    for (it[i][j] = grid[i][j].begin(); it[i][j] != grid[i][j].end(); it[i][j]++) {
                        // 如果找到了基体晶粒，并且其phi值大于0，说明基体还没有消失，我们记录下其数量
                        if (it[i][j]->key == N - 1 && it[i][j]->phi > 0) {
                            baseNum++;
                        }
                    }
                }
            }
            // 当基体的有效网格数量所占比值小于0.001，就认为晶体完全长大，这时跳出循环，进行再结晶的过程。
            if (double(baseNum)/(Nx * Ny) < 0.001) {
                break;
            }
        }
    }
    ...
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY | FOREGROUND_GREEN);
    cout << "Grain grown over!" << endl << endl;
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
    ...
    // 将晶粒的位错密度同样进行初始化，开始时是N - 1个晶粒(本程序是6个晶粒，基体晶粒已经消失)
    // 因为初始时均为rho0，所以直接初始化为rho0
    for (int i = 0; i < N - 1; i++) {
        grainRho = {i, rho0};
        // 将对应的结构体push到这个rhovec中
        rhovec.push_back(grainRho);
        rhovec_b.push_back(grainRho);
    }

    // 迭代器绑定
    itrho = rhovec.begin();
    itrho_b = rhovec_b.begin();
    ...
    // 每隔时间intervalTime生成一个文件，此时相当于每隔intervalTime / delteT步生成一个文件
    double intervalTime = 5.0;

    double timeFlag = 0.0;

    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY | FOREGROUND_GREEN);
    cout << "Second Step: begin to DRX!" << endl;
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
    cout << "Epsilon is " << epsilon << "/s, "
         << "deltaT is " << deltaT << ", all files will be created every " << intervalTime/deltaT << " steps." << endl;
    cout << "Please wait to compute..." << endl;
    int stepFlag = 0;
    ...
    while(true)
    {
        ...
        // 同样的，对于rhovec这个vector中的数据也是需要更新的!
        // ATTENTION: 在当前时间步即将结束时，需要将rhovec_b的所有值深克隆给rhovec，达到顺序一致性(因为计算过程中，可能会有晶粒的增加或者减少，导致顺序混乱)
        for (itrho = rhovec.begin(), itrho_b = rhovec_b.begin(); itrho != rhovec.end(); itrho++, itrho_b++) {
            itrho_b->rho = (k1 * pow(itrho->rho, 0.5) - k2 * itrho->rho) * (epsilon * deltaT) + itrho->rho;
        }
        ...
        if (refPointB > rhoc) {
            ...
            if (stepFlag > step)
            {
                ...
                if (ngb > 0) {
                    ...
                    newGrainNumber = newGrainNumber + 1; // 即统计的新生成的晶粒个数增加
                    grain = {N - 1, 1.0}; // 创建新的晶粒结构体，因为标号从0开始，所以，这里key是N - 1，因为是添加晶核，phi为1.0
                    cout << newGrainNumber << " grain created" << endl;
                    ...
                    for (int i = 0; i < Nx; i++)
                    {
                        for (int j = 0; j < Ny; j++)
                        {
                            int dif = pow((i - pos[index][0]), 2) + pow((j - pos[index][1]), 2) - pow(roundDRX, 2);
                            if (dif < 0 || dif == 0)
                            {
                    ...
                    // 生成新的晶粒之后，同时也需要在rhovec中添加一个新的结构体
                    grainRho = {N - 1, rho0};
                    rhovec_b.push_back(grainRho);
            ...
            for (int i = 0; i < Nx; i++)
            {
                for (int j = 0; j < Ny; j++)
                {
                    ...
                    for (it[i][j] = grid[i][j].begin(), it_b[i][j] = grid_b[i][j].begin(); it[i][j] != grid[i][j].end(); it[i][j]++, it_b[i][j]++)
                    {
                        ...
                        // 如果上一时刻为0，增量大于0，那么就说明这个网格点的这个晶粒的晶界在扩展 - 原来这个点在晶粒外，现在再晶界上。
                        // 基于同一个晶粒内部的rho值相同的原则，需要更新当前格点的rho值，
                        if ((it[i][j]->phi == 0) && (increment > 0))
                        {
                            double newRho = 0.0;
                            for (itrho_b = rhovec_b.begin(); itrho_b != rhovec_b.end(); itrho_b++)
                    ...
                    // 先判断晶粒总数是否大于5(我们指定的最大晶粒数)，如果大于则排序，否则不用排序！
        ...
        if (timeFlag > intervalTime)
        {
            cout << numberDRX - 99 << " drx files have been created, current time is " << int(curTimeDRX) << ". " << endl;
            output(phi_o, grid_b, it_b, N, Nx, Ny, numberDRX++);
            timeFlag = 0;
        }
    }
    cout << "Finished! " << endl;
