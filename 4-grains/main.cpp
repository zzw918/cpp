...
const int N = 4; // the total number of grains
// that Nx is 120 and Ny is 70 is fit
const int Nx = 100; // the x-axis grid numbers
const int Ny = 100; // the y-axis grid numbers
...
int main()
{
    ...
    double deltaT = 0.01,                 // timeInterval
           allTime = 4.5;                  // the whole time to grow
        ...
        for (int i = 0; i < Nx; i++)
        {
            for (int j = 0; j < Ny; j++)
            {
                ...
                switch (n) {
                    // new
                    case 0:
                        if ((i <= Nx / 2.0 + round && i >= Nx / 2.0 - round) && (j <= Ny / 2.0 + round && j >= Ny / 2.0 - round)) {
                    ...
                    case 1:
                        if ((i <= round || i >= Nx - round) && (j >= Ny / 4.0 - round && j <= Ny / 4.0 + round)) {
                    ...
                    case 2:
                        if ((i <= round || i >= Nx - round) && (j >= (Ny / 4.0) * 3 - round && j <= (Ny / 4.0) * 3 + round)) {
                    ...
                    case 3:
                        if ((i <= Nx / 2.0 + round && i >= Nx / 2.0 - round) && (j <= round || j >= Ny - round)) {
