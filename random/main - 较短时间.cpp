...
const int N = 11;    // the total number of grains
...
int main()
{
    cout << "Begin to calculate random 25-grains with kim algorithm!" << endl;
    ...
    vector<struct Position> allPos;
    vector<struct Position>::iterator itall;
    ...
    // for (itall = allPos.begin(); itall != allPos.end(); itall++) {
    //     cout << itall->randX << " " << itall->randY << endl; 
    // }
    // system("pause");
    for (int i = 0; i < Nx; i++) {
        for (int j = 0; j < Ny; j++) {
    ...
    double deltaT = 0.01, // timeInterval
        allTime = 120.0;   // the whole time to grow
